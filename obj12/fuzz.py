...
class Fuzzer:
    ...
    def setup(self):
        ...
        # Create RAM-backed input file under /tmp/<pid>/
        self.run_dir = f"/tmp/{os.getpid()}"
        os.makedirs(self.run_dir, exist_ok=True)
        self.input_path = os.path.join(self.run_dir, "input.txt")
        ...
        # Create file for later mmap interactions (now on tmpfs)
        with open(self.input_path, "wb") as f:
            f.truncate(SIZE)

        self.file = open(self.input_path, "r+b")
    ...
    def load_seeds(self, seeds_dir: str = "seeds"):
        ...
        for path in Path(seeds_dir).iterdir():
            if path.is_file():
                ...
                self.pool.filter(data)
                #######
                data2 = self.mutator.mutate_big(data)
                self.write_input(data2)
                # Execute target
                result = self.execute_target()
                self.pool.filter(data2)
                #######
                data2 = self.mutator.mutate_huge(data)
                self.write_input(data2)
                # Execute target
                result = self.execute_target()
                self.pool.filter(data2)
    ...
    def run(self):
        ...
        self.setup()
        try:
            posix_ipc.unlink_shared_memory("/fuzz_coverage_12")
